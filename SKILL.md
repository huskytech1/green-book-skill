...
- **本地化提炼规范**：
  ...
  - **标题处理**：同步完成 `coverTitle` 和 `headerTitle` 的语义化提炼。
    - **封面标题规范**：每条标题总字数不得超过 **20 个字符**。
      - 若总字数 **≤ 11 字**：采用**单行显示**，保持简洁。
      - 若总字数 **> 11 字**：强制**双行显示**（使用 `\n` 语义化断句），此时应尽可能扩充/修饰文案至 **15 字左右** 以保证视觉充实感。
      - **硬性约束**：无论单行双行，**每单行不得超过 11 个字符**。
